...
class MAMChannel {
  ...
  async publish(packet) {
    ...
    try {
      ...
      var prova = await MAM.attach(message.payload, message.address, 3, 14);

      //console.log(prova[prova.length-1].attachmentTimestamp)
      //prova[0].attachmentTimestamp

    } catch (e) {
      //console.log(e);
    }
    var res_array = []
    //res_array.push(prova[0].attachmentTimestamp)
    res_array.push(message.root)
